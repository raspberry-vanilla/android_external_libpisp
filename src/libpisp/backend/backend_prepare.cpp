...
void check_stride(pisp_image_format_config const &config)
{
	if (config.stride % PISP_BACK_END_OUTPUT_MIN_ALIGN || config.stride2 % PISP_BACK_END_OUTPUT_MIN_ALIGN)
		abort(); //throw std::runtime_error("Output stride values not sufficiently aligned");

	if (PISP_IMAGE_FORMAT_wallpaper(config.format) && (config.stride % 128 || config.stride2 % 128))
		abort(); //throw std::runtime_error("Wallpaper format should have 128-byte aligned rolls");
...
void finalise_bayer_rgb_inputs(pisp_image_format_config const &config)
{
	if (config.width < PISP_BACK_END_MIN_TILE_WIDTH || config.height < PISP_BACK_END_MIN_TILE_HEIGHT)
		abort(); //throw std::runtime_error("finalise_bayer_rgb_inputs: input image too small");
}
...
void finalise_inputs(pisp_be_config &config)
{
	...
	if (config.global.bayer_enables & PISP_BE_BAYER_ENABLE_INPUT)
	{
		if ((config.input_format.width & 1) || (config.input_format.height & 1))
			abort(); //throw std::runtime_error("finalise_inputs: Bayer pipe image dimensions must be even");
		if (config.input_format.stride & 15)
			abort(); //throw std::runtime_error("finalise_inputs: input stride should be at least 16-byte aligned");
	}
	else if (config.global.rgb_enables & PISP_BE_RGB_ENABLE_INPUT)
	{
		if (PISP_IMAGE_FORMAT_sampling_420(config.input_format.format) && (config.input_format.width & 1))
			abort(); //throw std::runtime_error("finalise_inputs: 420 input height must be even");
			...
			abort(); //throw std::runtime_error("finalise_inputs: 420/422 input width must be even");
		if (PISP_IMAGE_FORMAT_wallpaper(config.input_format.format))
		{
			if ((config.input_format.stride & 127) || (config.input_format.stride2 & 127))
				abort(); //throw std::runtime_error("finalise_inputs: wallpaper format strides must be at least 128-byte aligned");
		}
		else if ((config.input_format.stride & 15) || (config.input_format.stride2 & 15))
			abort(); //throw std::runtime_error("finalise_inputs: input strides must be at least 16-byte aligned");
	}
}
					   ...
					   uint16_t height)
{
	...
	if ((scale_factor_h < UnityScale / 16 || scale_factor_h >= 16 * UnityScale) ||
		...
		abort(); //throw std::runtime_error("finalise_resample: Invalid scaling factors (must be < 16x down/upscale).");
						...
						uint16_t height)
{
	...
	if ((scale_factor_h != UnityScale && (scale_factor_h < 2 * UnityScale || scale_factor_h > 8 * UnityScale)) ||
		...
		abort(); //throw std::runtime_error("finalise_downscale: Invalid scaling factors (must be 1x or >= 2x && <= 8x).");
...
void finalise_decompression(pisp_be_config const &be_config)
{
	...
	if (PISP_IMAGE_FORMAT_compressed(fmt) && !(bayer_enables & PISP_BE_BAYER_ENABLE_DECOMPRESS))
		abort(); //throw std::runtime_error("BackEnd::finalise: input compressed but decompression not enabled");

	if (!PISP_IMAGE_FORMAT_compressed(fmt) && (bayer_enables & PISP_BE_BAYER_ENABLE_DECOMPRESS))
		abort(); //throw std::runtime_error("BackEnd::finalise: input uncompressed but decompression enabled");

	if ((bayer_enables & PISP_BE_BAYER_ENABLE_DECOMPRESS) && !PISP_IMAGE_FORMAT_bps_8(fmt))
		abort(); //throw std::runtime_error("BackEnd::finalise: compressed input is not 8bpp");
}
...
static void check_rawio_format(pisp_image_format_config &fmt, uint16_t w, uint16_t h)
{
	if (fmt.width == 0 || fmt.height == 0)
	{
		...
		abort(); //throw std::runtime_error("BackEnd::finalise: Image dimensions do not match input");
...
void finalise_tdn(pisp_be_config &config)
{
	...
	if (tdn_enabled && !tdn_output_enabled)
		abort(); //throw std::runtime_error("BackEnd::finalise: TDN output not enabled when TDN enabled");

	if (PISP_IMAGE_FORMAT_compressed(fmt) && !tdn_compress_enabled)
		abort(); //throw std::runtime_error("BackEnd::finalise: TDN output compressed but compression not enabled");

	if (!PISP_IMAGE_FORMAT_compressed(fmt) && tdn_compress_enabled)
		abort(); //throw std::runtime_error("BackEnd::finalise: TDN output uncompressed but compression enabled");

	if (tdn_compress_enabled && !PISP_IMAGE_FORMAT_bps_8(fmt))
		abort(); //throw std::runtime_error("BackEnd::finalise: TDN output does not match compression mode");
	...
	{
		if (tdn_input_enabled)
			abort(); //throw std::runtime_error("BackEnd::finalise: TDN input enabled but TDN not enabled");
	...
	{
		if (tdn_input_enabled)
			abort(); //throw std::runtime_error("BackEnd::finalise: TDN input enabled but TDN being reset");
	}
	else
	{
		if (!tdn_input_enabled)
			abort(); //throw std::runtime_error("BackEnd::finalise: TDN input not enabled but TDN not being reset");
		...
		if (PISP_IMAGE_FORMAT_compressed(fmt) && !tdn_decompress_enabled)
			abort(); //throw std::runtime_error("BackEnd::finalise: TDN input compressed but decompression not enabled");
		if (!PISP_IMAGE_FORMAT_compressed(fmt) && tdn_decompress_enabled)
			abort(); //throw std::runtime_error("BackEnd::finalise: TDN input uncompressed but decompression enabled");
		if (tdn_compress_enabled && !PISP_IMAGE_FORMAT_bps_8(fmt))
			abort(); //throw std::runtime_error("BackEnd::finalise: TDN output does not match compression mode");
	}
}
...
void finalise_stitch(pisp_be_config &config)
{
	...
	if (stitch_enabled != stitch_input_enabled)
		abort(); //throw std::runtime_error("BackEnd::finalise: stitch and stitch_input should be enabled/disabled together");
	if (stitch_input_enabled && PISP_IMAGE_FORMAT_compressed(input_fmt) && !stitch_decompress_enabled)
		abort(); //throw std::runtime_error("BackEnd::finalise: stitch output compressed but decompression not enabled");
	if (stitch_input_enabled && !PISP_IMAGE_FORMAT_compressed(input_fmt) && stitch_decompress_enabled)
		abort(); //throw std::runtime_error("BackEnd::finalise: stitch output uncompressed but decompression enabled");
	if (stitch_output_enabled && PISP_IMAGE_FORMAT_compressed(output_fmt) && !stitch_compress_enabled)
		abort(); //throw std::runtime_error("BackEnd::finalise: stitch output compressed but compression not enabled");
	if (stitch_output_enabled && !PISP_IMAGE_FORMAT_compressed(output_fmt) && stitch_compress_enabled)
		abort(); //throw std::runtime_error("BackEnd::finalise: stitch output uncompressed but compression enabled");
	if (stitch_decompress_enabled && !PISP_IMAGE_FORMAT_bps_8(input_fmt))
		abort(); //throw std::runtime_error("BackEnd::finalise: stitch input does not match compression mode");
	if (stitch_compress_enabled && !PISP_IMAGE_FORMAT_bps_8(output_fmt))
		abort(); //throw std::runtime_error("BackEnd::finalise: stitch output does not match compression mode");
...
void finalise_output(pisp_be_output_format_config &config)
{
	...
	if (config.image.width < PISP_BACK_END_MIN_TILE_WIDTH || config.image.height < PISP_BACK_END_MIN_TILE_HEIGHT)
		abort(); //throw std::runtime_error("finalise_output: output image too small");

	if (PISP_IMAGE_FORMAT_sampling_420(config.image.format) && (config.image.height & 1))
		abort(); //throw std::runtime_error("finalise_output: 420 image height should be even");
	...
	if ((PISP_IMAGE_FORMAT_sampling_420(config.image.format) || PISP_IMAGE_FORMAT_sampling_422(config.image.format)) &&
		...
		abort(); //throw std::runtime_error("finalise_output: 420/422 image width should be even");

	if (PISP_IMAGE_FORMAT_wallpaper(config.image.format))
	{
		if ((config.image.stride & 127) || (config.image.stride2 & 127))
			abort(); //throw std::runtime_error("finalise_output: wallpaper image stride should be at least 128-byte aligned");
	}
	else if ((config.image.stride & 15) || (config.image.stride2 & 15))
		abort(); //throw std::runtime_error("finalise_output: image stride should be at least 16-byte aligned");
}
		 ...
		 TilingConfig const &tiling_config)
{
	...
	for (auto &tile : tiles)
	{
		...
		if (tile.input_width < PISP_BACK_END_MIN_TILE_WIDTH || tile.input_height < PISP_BACK_END_MIN_TILE_HEIGHT)
			abort(); //throw std::runtime_error("Tile too small at input");
		...
		for (unsigned int i = 0; i < numBranches; i++)
		{
			...
			if (width_after_crop && height_after_crop)
			{
				...
				if (width_after_crop < PISP_BACK_END_MIN_TILE_WIDTH)
				{
					...
					if (!rh_edge)
						abort(); //throw std::runtime_error("Tile width too small after crop");
				}
				if (height_after_crop < PISP_BACK_END_MIN_TILE_HEIGHT)
					abort(); //throw std::runtime_error("Tile height too small after crop");
				...
				if (tile.resample_in_width[i] < PISP_BACK_END_MIN_TILE_WIDTH)
				{
					...
					if (!rh_edge)
						abort(); //throw std::runtime_error("Tile width too small after downscale");
				}

				if (tile.resample_in_height[i] < PISP_BACK_END_MIN_TILE_HEIGHT)
					abort(); //throw std::runtime_error("Tile height too small after downscale");

				if (!rh_edge && tile.output_width[i] < PISP_BACK_END_MIN_TILE_WIDTH)
					abort(); //throw std::runtime_error("Tile width too small at output");
				if (tile.output_height[i] < PISP_BACK_END_MIN_TILE_HEIGHT)
					abort(); //throw std::runtime_error("Tile height too small at output");
			}
		}
		tile_num++;
	}
...
void BackEnd::finaliseConfig()
{
		...
		if (enabled)
		{
			...
			if (dirty_flags_rgb & PISP_BE_RGB_ENABLE_DOWNSCALE(j))
			{
				if (variant_.BackEndDownscalerAvailable(0, j))
					...
					abort(); //throw std::runtime_error("Downscale is not available in output branch " + std::to_string(j));
			}
	...
	if (!((be_config_.global.bayer_enables & PISP_BE_BAYER_ENABLE_INPUT) || (be_config_.global.bayer_enables == 0)))
		abort(); //throw std::runtime_error("BackEnd::finalise: Bayer input disabled but Bayer pipe active");

	if (!!(be_config_.global.bayer_enables & PISP_BE_BAYER_ENABLE_INPUT) +
			!!(be_config_.global.rgb_enables & PISP_BE_RGB_ENABLE_INPUT) !=
		1)
		abort(); //throw std::runtime_error("BackEnd::finalise: exactly one of Bayer and RGB inputs should be enabled");
	...
	if (output_enables == 0)
		abort(); //throw std::runtime_error("BackEnd::finalise: PiSP not configured to do anything");
}
...
std::vector<pisp_tile> BackEnd::retilePipeline(TilingConfig const &tiling_config)
{
	...
	for (int i = 0; i < num_tiles_x_ * num_tiles_y_; i++)
	{
		...
		if (tiles[i].input.output != tiles[i].input.input)
			abort(); //throw std::runtime_error("BackEnd::retilePipeline: tiling error in Bayer pipe");
			...
			if (be_config_.global.rgb_enables & PISP_BE_RGB_ENABLE_RESAMPLE(j))
			{
				...
				if (std::abs(t.resample_phase_x[0 * variant_.BackEndNumBranches(0) + j] -
					...
					std::abs(t.resample_phase_x[0 * variant_.BackEndNumBranches(0) + j] -
							 t.resample_phase_x[2 * variant_.BackEndNumBranches(0) + j]) > phase_max)
				{
					abort(); //throw std::runtime_error("Resample phase x for tile is > 0.5 pixels on the output dimensions.");
				}
				...
				if (std::abs(t.resample_phase_y[0 * variant_.BackEndNumBranches(0) + j] -
					...
					std::abs(t.resample_phase_y[0 * variant_.BackEndNumBranches(0) + j] -
							 t.resample_phase_y[2 * variant_.BackEndNumBranches(0) + j]) > phase_max)
				{
					abort(); //throw std::runtime_error("Resample phase y for tile is > 0.5 pixels on the output dimensions.");
				}
			}
		}
	}
...
void BackEnd::Prepare(pisp_be_tiles_config *config)
{
	...
	if ((be_config_.global.bayer_enables & PISP_BE_BAYER_ENABLE_INPUT) == 0 &&
		...
		abort(); //throw std::runtime_error("BackEnd::preFrameUpdate: neither Bayer nor RGB inputs are enabled");
	else if ((be_config_.global.bayer_enables & PISP_BE_BAYER_ENABLE_INPUT) &&
			 (be_config_.global.rgb_enables & PISP_BE_RGB_ENABLE_INPUT))
		abort(); //throw std::runtime_error("BackEnd::preFrameUpdate: both Bayer and RGB inputs are enabled");
	...
	for (unsigned int i = 0; i < variant_.BackEndNumBranches(0); i++)
	{
		...
		if (image_config.format & PISP_IMAGE_FORMAT_INTEGRAL_IMAGE)
		{
			if (!variant_.BackEndIntegralImage(0, i))
				abort(); //throw std::runtime_error("Integral images are not supported in the current configuration.");
